...
- Do not duplicate Claude agents or skills under `.codex/`. The source of truth for Claude-owned agents and skills remains `.claude/`. Codex-only orchestration skills may live under `.codex/skills/` when they reference canonical Claude resources rather than copying them.
- Code-review runtimes share collision-safe artifacts under each target repository's `.code-review/runs/<run-id>/` directory.
...
- Codex SessionStart warns when dotfiles symlinks drift and restores capped original/latest user requests from rollout JSONL after compaction. Claude's model-lock mutation has no Codex counterpart; static reasoning config is authoritative. Superpowers reinjection remains excluded. It also warns when `approvals_reviewer`/`default_permissions` drift from their configured values (Codex persists session state back into `config.toml` and has flipped `approvals_reviewer` to `"user"` before).
- Claude WebFetch domain allowlists are not a direct Codex web-search control. Sandboxed shell networking is governed by the `workspace-net` permission profile. If a session hits `could not resolve host: github.com` / `gh` failing to reach `api.github.com` / `.git` refusing writes, the session lost network access mid-run — not a DNS or allowlist fault. Switching approval or permission mode from the TUI mode picker mid-session replaces `workspace-net` with a built-in preset (network off, `.git` read-only); the picker cannot re-select a custom `default_permissions` profile. Remedy: start a new session rather than retrying with escalation. Confirm via `sqlite3 ~/.codex/state_5.sqlite "select id,cwd,substr(sandbox_policy,1,80) from threads order by updated_at desc limit 5;"` — `special/root` means downgraded, `path:"/"` means healthy `workspace-net`.
