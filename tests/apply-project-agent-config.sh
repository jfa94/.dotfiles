...
#!/usr/bin/env bash
set -euo pipefail
[[ "\$1" == mcp && "\$2" == add-json && "\$3" == --scope && "\$4" == user ]] || { echo "unexpected claude invocation: \$*" >&2; exit 1; }
name="\$5" json="\$6"
...
jq -e '.mcpServers.posthog_write.headersHelper | contains("PostHog Outsidey API Key")' "$SCRATCH/outsidey/.mcp.json" >/dev/null
grep -Fqx 'POSTHOG_CLI_PROJECT_ID=107700' "$SCRATCH/outsidey/.agent-env"

# User-scope registration: read-only, not project-pinned, on the generic (non-Outsidey) credential.
jq -e '.mcpServers.posthog.url == "https://mcp.posthog.com/mcp?mode=cli&readonly=true"' "$STUB_CLAUDE_JSON" >/dev/null
...
jq -e '.mcpServers.posthog_write.url == "https://mcp.posthog.com/mcp?mode=cli"' "$STUB_CLAUDE_JSON" >/dev/null
