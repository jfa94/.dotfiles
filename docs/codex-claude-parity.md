...
- Codex config sets `gpt-5.6-sol` as the default, with `medium` reasoning and `xhigh` in Plan mode. There is no startup model-lock mutation; the configured default is not an enforced session lock.
