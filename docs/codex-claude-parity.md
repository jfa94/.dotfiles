...
- `dangerous-patterns-check.sh` auto-allows a single literal `rm` only when every operand resolves to either a subpath of `/tmp`, `/private/tmp`, or `/var/tmp`, or a path physically inside the current Git repository that is both Git-ignored and beneath an exact known artifact segment. The named segments cover the existing cache/build/test directories plus `node_modules`, `.venv`, `target`, `tmp`, and `.pytest_cache`; Git-ignore status alone is intentionally insufficient because repositories also ignore durable local state and secrets. Tracked artifacts, unnamed ignored paths, secret-shaped paths, globs, expansions, and compounds retain confirmation gating. Traversal, bare tmp roots, and unquoted paths physically outside the repository remain hard denials, including intermediate and trailing-slash symlink escapes; the pre-existing quoted-operand backstop still falls through to confirmation. Claude emits an explicit `permissionDecision:"allow"`; Codex exits 0 and leaves its unchanged rules layer to `auto_review`. The mirrored constants and behavior are pinned by `tests/codex-permissions-aws-mcp.sh` and `tests/dangerous-patterns.sh`.
